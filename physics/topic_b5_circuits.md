...
### Electric Current
- **Current** ($I$): rate of flow of charge; $I = \frac{\Delta q}{\Delta t}$
- SI unit: ampere (A) = C/s
- Conventional current: direction of positive charge flow (opposite to electron flow)
- **Charge carriers**: electrons in metals, ions in electrolytes

### Potential Difference (Voltage)
- **Potential difference** ($V$): energy transferred per unit charge; $V = \frac{W}{q}$
- SI unit: volt (V) = J/C
- **EMF** ($\varepsilon$): energy per unit charge provided by a source

### Resistance and Ohm's Law
- **Resistance** ($R$): opposition to current flow; $R = \frac{V}{I}$
- SI unit: ohm ($\Omega$)
- **Ohm's law**: $V = IR$ (for ohmic conductors: linear V-I relationship)
